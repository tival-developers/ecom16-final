...
export async function getHomePageProducts() {
  ...
  try {
    ...
    const recentRaw = await Product.find().sort({ createdAt: -1 }).limit(5).lean()
    const trendingRaw = await Product.find({ isTrending: true }).sort({ createdAt: -1 }).limit(4).lean()
    const featuredRaw = await Product.find({ isFeatured: true }).sort({ createdAt: -1 }).limit(5).lean()
